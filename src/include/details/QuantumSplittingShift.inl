...
advance(Container& con1, Container& con2) {
  ...
  std::ofstream out("test_phase"+std::to_string(p.mpi_rank),std::ios::out);
  ...
  for (idx_type i = p.nxbd; i<p.nxbd+p.nxloc; i++) {
    ...
    transform(i_begin,i_begin+n_chunk,
              ...
              { return qDt*(tex1D<float>(tex,(X-l*qDl))
                           -tex1D<float>(tex,(X+l*qDl))); });
    /*
    thrust::copy(phase.begin(),phase.end(),
                 std::ostream_iterator<val_type>(out," "));
    out << std::endl;*/
