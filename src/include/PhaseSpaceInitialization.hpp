...
struct Idx2Value {
	...
	__host__ __device__ 
	Idx2Value(idx_array n_dim, val_array lb, val_array h)
	...
	val_type operator()(const idx_type& idx) {
		...
		return   std::exp(-std::pow(low_bound[0]+h[0]*idx_m[2]-5,2))
		       * std::exp(-std::pow(low_bound[1]+h[1]*idx_m[3]-5,2))
		       * std::exp(-std::pow(low_bound[2]+h[2]*idx_m[0],2))
		       * std::exp(-std::pow(low_bound[3]+h[3]*idx_m[1],2));
	}
...
public:
	...
	void operator()(const ExecutionPolicy & exec,
		...
		std::array<idx_type,dim> n_dim_init;
		thrust::scatter(p->n.begin(),p->n.end(),
		                p->init_order.begin(),n_dim_init.begin());
		thrust::transform(exec, 
		                  thrust::make_counting_iterator(start),
		                  thrust::make_counting_iterator(num+start), 
		                  itor_begin,
		                  Idx2Value(n_dim_init,p->low_bound,p->interval));
